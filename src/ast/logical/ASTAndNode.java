...
public final class ASTAndNode implements JasvaASTNode {

    private final JasvaASTNode _left;
    private final JasvaASTNode _right;

    public ASTAndNode(final JasvaASTNode left, final JasvaASTNode right) {
        _left = left;
        _right = right;
    }
    ...
    public final Value<?> evaluate(Environment<Value<?>> symbolTable) {
        ...
        if(leftVal instanceof BoolValue) {
            ...
            if(rightVal instanceof BoolValue) {
                return ((BoolValue) leftVal).and((BoolValue) rightVal);
            }
            throw new TypeException("boolean and", "right hand side is not a boolean value");
        }
        throw new TypeException("boolean and", "left hand side is not a boolean value");
    }
    ...
    public final void compile(CompiledClass<JasminCompiler> classContext, JasminCompiler compiler, Environment<Pair<Integer, String>> symbolLinker) {
        ...
        compiler.pushInstruction(JasvaInstructionSet.AND.instance());
    }
