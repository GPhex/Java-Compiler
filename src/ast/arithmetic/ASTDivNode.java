...
public final class ASTDivNode implements JasvaASTNode {

    private final JasvaASTNode _left;
    private final JasvaASTNode _right;

    public ASTDivNode(final JasvaASTNode left, final JasvaASTNode right) {
        _left = left;
        _right = right;
    }
    ...
    public final Value<?> evaluate(Environment<Value<?>> symbolTable) {
        ...
        if(leftVal instanceof IntValue) {
            ...
            if(rightVal instanceof IntValue) {
                return ((IntValue) leftVal).div((IntValue) rightVal);
            }
            throw new TypeException("division", "right hand side is not an integer value");
        }
        throw new TypeException("division", "left hand side is not an integer value");
    }
    ...
    public final void compile(CompiledClass<JasminCompiler> classContext, JasminCompiler compiler, Environment<Pair<Integer, String>> symbolLinker) {
        ...
        compiler.pushInstruction(JasvaInstructionSet.DIV.instance());
    }
